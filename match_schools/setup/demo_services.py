...
def _books() -> int:
	...
	for title, author, isbn, category, copies in BOOKS:
		if frappe.db.exists("MS Library Book", {"isbn": isbn}):
			continue
		frappe.get_doc(
			{
				"doctype": "MS Library Book",
				"title": title,
				"author": author,
				"isbn": isbn,
				"category": category,
...
def _routes() -> int:
	...
	for name, driver, phone, capacity, fee in ROUTES:
		if frappe.db.exists("MS Transport Route", {"route_name": name}):
			continue
		frappe.get_doc(
			{
				"doctype": "MS Transport Route",
				"route_name": name,
				"driver_name": driver,
				"driver_phone": phone,
...
def _loans(students: list[dict]) -> int:
	books = frappe.get_all("MS Library Book", fields=["name", "available_copies"], limit=20)
	if not books:
		return 0
	...
	for student in students[:24]:
		...
		if frappe.db.exists(
			"MS Book Loan", {"student": student.name, "book": book.name, "status": "Issued"}
		):
			continue
...
def _transport(students: list[dict]) -> int:
	...
	routes = []
	for r in frappe.get_all("MS Transport Route", fields=["name", "capacity"]):
		used = frappe.db.count("MS Transport Assignment", {"route": r.name, "active": 1})
	...
	for student in students[:35]:
		if frappe.db.exists("MS Transport Assignment", {"student": student.name}):
			continue
		...
		frappe.get_doc(
			{
				"doctype": "MS Transport Assignment",
				"student": student.name,
				"route": route["name"],
				"stop": random.choice(
...
def _health(students: list[dict]) -> int:
	...
	for student in students[:40]:
		if frappe.db.exists("MS Health Record", {"student": student.name}):
			continue
		frappe.get_doc(
			{
				"doctype": "MS Health Record",
				"student": student.name,
