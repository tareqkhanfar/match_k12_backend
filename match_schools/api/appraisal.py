...
def list_observations(
	...
	total = frappe.db.count("MS Teacher Observation", filters)
...
def get_observation(observation: str, persona: str = None):
	...
	doc = frappe.get_doc("MS Teacher Observation", observation)
...
def save_observation(payload: str | dict, persona: str = None):
	...
	doc = (
		frappe.get_doc("MS Teacher Observation", observation_id)
		if observation_id
		else frappe.new_doc("MS Teacher Observation")
	)
...
def delete_observation(observation: str, persona: str = None):
	status = frappe.db.get_value("MS Teacher Observation", observation, "status")
	if status == "Acknowledged":
		return fail(
			message_en="An acknowledged observation cannot be deleted.",
			message_ar="لا يمكن حذف تقييم اطّلع عليه المعلم.",
		)
	frappe.delete_doc("MS Teacher Observation", observation, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": observation},
...
def performance_file(instructor: str = None, persona: str = None):
	...
	marks_entered = frappe.db.count(
		"MS Gradebook Entry", {"entered_by": frappe.db.get_value("Instructor", instructor, "name")}
	)
	assignments = frappe.db.count("MS Assignment", {"instructor": instructor})
