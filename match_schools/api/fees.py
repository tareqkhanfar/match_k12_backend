...
def statement(student: str = None, persona: str = None):
	"""كشف الحساب. Built from the student's customer ledger — see `statement.py`."""
	from match_schools.api import statement as account_statement

	scope = resolve_scope(persona)
	students = account_statement.students_for(persona, scope, student)
	if not students:
		return account_statement.family([], None)
	if persona in (ROLE_ADMIN, ROLE_SECRETARY):
		return account_statement.family(students, students[0])
	selected = student if student in students else (students[0] if len(students) == 1 else None)
	return account_statement.family(students, selected)
