...
class K12AssignmentSubmission(Document):
	def validate(self):
	...
	def validate_score(self):
		...
		maximum = self.maximum_score or frappe.db.get_value(
			"K12 Assignment", self.assignment, "maximum_score"
		)
	...
	def set_late_status(self):
		...
		if self.status not in ("Submitted", "Late"):
			return
		due_date = frappe.db.get_value("K12 Assignment", self.assignment, "due_date")
