...
"""Seed realistic Arabic demo data for the Match K12 frontend.

Run with:
    bench --site <site> execute match_k12.setup.demo_data.create_demo_data
"""
...
def _ensure_assignments(groups, instructors, courses, students):
	...
	for i, title in enumerate(titles):
		...
		if frappe.db.exists("K12 Assignment", {"title": title, "student_group": group["name"]}):
			continue
		frappe.get_doc(
			{
				"doctype": "K12 Assignment",
				"title": title,
...
def _ensure_announcements():
	...
	for i, (title, body, atype, audience) in enumerate(items):
		if frappe.db.exists("K12 Announcement", {"title": title}):
			continue
		frappe.get_doc(
			{
				"doctype": "K12 Announcement",
				"title": title,
				"body": f"<p>{body}</p>",
