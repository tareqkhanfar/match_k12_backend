...
def list_quizzes(
	...
	total = frappe.db.count("K12 Quiz", filters)
	...
	if rows:
		for q in frappe.get_all(
			"K12 Quiz Question",
			filters={"parent": ["in", [r.name for r in rows]], "parenttype": "K12 Quiz"},
			fields=["parent"],
		):
...
def save_quiz(payload: str | dict, persona: str = None):
	...
	quiz_id = data.get("id")
	doc = frappe.get_doc("K12 Quiz", quiz_id) if quiz_id else frappe.new_doc("K12 Quiz")
	...
	if quiz_id and doc.status == "Published" and data.get("questions") is not None:
		if frappe.db.exists("K12 Quiz Attempt", {"quiz": quiz_id}):
			return fail(
...
def get_quiz(quiz: str, persona: str = None):
	...
	doc = frappe.get_doc("K12 Quiz", quiz)
...
def delete_quiz(quiz: str, persona: str = None):
	if frappe.db.exists("K12 Quiz Attempt", {"quiz": quiz}):
		return fail(
	...
	frappe.delete_doc("K12 Quiz", quiz, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": quiz},
...
def start_attempt(quiz: str, persona: str = None):
	...
	if live:
		attempt = frappe.get_doc("K12 Quiz Attempt", live.name)
	else:
		...
		attempt = frappe.new_doc("K12 Quiz Attempt")
		attempt.quiz = quiz
...
def submit_attempt(attempt: str, answers: str | list, persona: str = None):
	...
	scope = resolve_scope(persona)
	doc = frappe.get_doc("K12 Quiz Attempt", attempt)
	...
	quiz = frappe.get_doc("K12 Quiz", doc.quiz)
