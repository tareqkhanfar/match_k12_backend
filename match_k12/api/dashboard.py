...
def _instructor_assignments(instructor: str | None) -> list[str]:
	...
	return [
		r.name
		for r in frappe.get_all("K12 Assignment", filters={"instructor": instructor}, fields=["name"])
	]
...
def _assignment_card(r: dict) -> dict:
	...
	submitted = frappe.db.count("K12 Assignment Submission", {"assignment": r.get("name")})
	return {
		"id": r.get("name"),
		"title": r.get("title"),
