...
def get_survey(survey: str, persona: str = None):
	...
	doc = frappe.get_doc("K12 Survey", survey)
...
def save_survey(payload: str | dict, persona: str = None):
	...
	doc = (
		frappe.get_doc("K12 Survey", data["id"])
		if data.get("id")
		else frappe.new_doc("K12 Survey")
	)
	...
	if data.get("id") and data.get("questions") is not None:
		if frappe.db.exists("K12 Survey Response", {"survey": data["id"]}):
			return fail(
...
def delete_survey(survey: str, persona: str = None):
	if frappe.db.exists("K12 Survey Response", {"survey": survey}):
		return fail(
			message_en="Responses exist; close the survey instead of deleting it.",
			message_ar="توجد إجابات — أغلق الاستبيان بدل حذفه.",
		)
	frappe.delete_doc("K12 Survey", survey, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": survey},
...
@frappe.whitelist()
@k12_endpoint(ROLE_TEACHER, ROLE_STUDENT, ROLE_PARENT, ROLE_ADMIN, ROLE_SECRETARY)
def submit_response(survey: str, answers: str | list, persona: str = None):
	...
	if not doc.anonymous and frappe.db.exists(
		"K12 Survey Response", {"survey": survey, "respondent": frappe.session.user}
	):
		return fail(
	...
	response = frappe.new_doc("K12 Survey Response")
...
def survey_results(survey: str, persona: str = None):
	...
	answers = frappe.get_all(
		...
		filters={
			...
			"parenttype": "K12 Survey Response",
		},
