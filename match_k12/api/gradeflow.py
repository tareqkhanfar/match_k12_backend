...
def _submission(student_group: str, course: str, academic_term: str | None):
	...
	return frappe.get_doc("K12 Term Submission", name) if name else None
...
def submit_term(
	...
	if not doc:
		doc = frappe.new_doc("K12 Term Submission")
...
def review_term(submission: str, action: str, notes: str = None, persona: str = None):
	...
	doc = frappe.get_doc("K12 Term Submission", submission)
...
def publish_term(
	...
	for s in submissions:
		if s.status == "Published":
			continue
		doc = frappe.get_doc("K12 Term Submission", s.name)
...
def unpublish_term(
	...
	for name in rows:
		doc = frappe.get_doc("K12 Term Submission", name)
		doc.status = "Approved"
