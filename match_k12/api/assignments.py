	...
	if existing:
		for f in frappe.get_all(
			...
			filters={
				...
				"parenttype": "K12 Assignment Submission",
			},
...
def submit_assignment(
	...
	group, due_date, status = frappe.db.get_value(
		"K12 Assignment", assignment, ["student_group", "due_date", "status"]
	)
...
@frappe.whitelist()
@k12_endpoint(ROLE_ADMIN, ROLE_SECRETARY, ROLE_TEACHER, ROLE_STUDENT)
...
def get_submission(assignment: str, student: str = None, persona: str = None):
	...
	submission = None
	if submission_name:
		doc = frappe.get_doc("K12 Assignment Submission", submission_name)
		submission = {
			"id": doc.name,
			"status": doc.status,
			"status_label": SUBMISSION_STATUS_AR.get(doc.status, doc.status),
	...
	return {
		"assignment": {
			...
			"files": [
				...
				for f in frappe.get_doc("K12 Assignment", assignment).files
			],
		},
