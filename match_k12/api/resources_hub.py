...
def save_resource(payload: str | dict, persona: str = None):
	...
	doc = (
		frappe.get_doc("K12 Resource", data["id"])
		if data.get("id")
		else frappe.new_doc("K12 Resource")
	)
...
def delete_resource(resource: str, persona: str = None):
	course = frappe.db.get_value("K12 Resource", resource, "course")
	if persona == ROLE_TEACHER:
		from match_k12.api.gradeflow import assert_teacher_owns_course

		assert_teacher_owns_course(persona, course)

	frappe.delete_doc("K12 Resource", resource, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": resource},
