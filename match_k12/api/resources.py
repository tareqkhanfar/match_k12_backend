...
def save_book(payload: str | dict, persona: str = None):
	...
	if book_id:
		doc = frappe.get_doc("K12 Library Book", book_id)
		doc.update(fields)
		doc.save()
		msg_en, msg_ar = "Book updated.", "تم تحديث الكتاب."
	else:
		doc = frappe.get_doc({"doctype": "K12 Library Book", **fields})
		doc.insert()
...
def delete_book(book: str, persona: str = None):
	on_loan = frappe.db.count("K12 Book Loan", {"book": book, "status": ["in", ["Issued", "Overdue"]]})
	if on_loan:
		return fail(
			message_en="This book still has copies on loan.",
			message_ar="لا يمكن الحذف: توجد نسخ معارة من هذا الكتاب.",
		)
	frappe.delete_doc("K12 Library Book", book)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": book},
...
def return_book(loan: str, status: str = "Returned", persona: str = None):
	...
	doc = frappe.get_doc("K12 Book Loan", loan)
	doc.status = status
	if status == "Returned":
...
def list_routes(filters: str | dict = None, persona: str = None):
	...
	out = []
	for r in rows:
		assigned = frappe.db.count("K12 Transport Assignment", {"route": r.name, "active": 1})
		out.append(
			{
				"id": r.name,
...
def save_route(payload: str | dict, persona: str = None):
	...
	if route_id:
		doc = frappe.get_doc("K12 Transport Route", route_id)
		doc.update(fields)
		doc.save()
		msg_en, msg_ar = "Route updated.", "تم تحديث الخط."
	else:
		doc = frappe.get_doc({"doctype": "K12 Transport Route", **fields})
		doc.insert()
...
def delete_route(route: str, persona: str = None):
	assigned = frappe.db.count("K12 Transport Assignment", {"route": route, "active": 1})
	if assigned:
		return fail(
			message_en="This route still has active student assignments.",
			message_ar="لا يمكن الحذف: يوجد طلاب مسندون لهذا الخط.",
		)
	frappe.delete_doc("K12 Transport Route", route)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": route},
...
def list_transport_assignments(
	...
	labels = {
		n: frappe.db.get_value("K12 Transport Route", n, "route_name") for n in route_names
	}
...
def save_transport_assignment(payload: str | dict, persona: str = None):
	...
	if assignment_id:
		doc = frappe.get_doc("K12 Transport Assignment", assignment_id)
		doc.update(fields)
		doc.save()
		msg_en, msg_ar = "Assignment updated.", "تم تحديث الإسناد."
	else:
		doc = frappe.get_doc({"doctype": "K12 Transport Assignment", **fields})
		doc.insert()
...
def delete_transport_assignment(assignment: str, persona: str = None):
	frappe.delete_doc("K12 Transport Assignment", assignment)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": assignment},
