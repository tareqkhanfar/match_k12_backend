...
student against them. A match raises a K12 Student Alert, which becomes the
...
def _term_grades_by_student(students: list[str]) -> dict[str, list]:
	...
	from match_k12.api.gradebook import _compute_subject_grade
...
def _measure_missing_assignments(students: list[str], rule) -> dict[str, float]:
	...
	assignments = frappe.get_all(
		"K12 Assignment", filters=filters, fields=["name", "student_group"], limit=2000
	)
	if not assignments:
		return {}
...
def evaluate_rule(rule) -> dict:
	...
	for student, value in matched.items():
		...
		if existing:
			frappe.db.set_value(
				"K12 Student Alert", existing, "measured_value", flt(value), update_modified=False
			)
			continue
		...
		doc = frappe.new_doc("K12 Student Alert")
		doc.student = student
		doc.rule = rule.name
	...
	for alert in frappe.get_all(
		...
		if alert.student not in matched:
			doc = frappe.get_doc("K12 Student Alert", alert.name)
			doc.status = "Resolved"
...
def escalate_open_alerts() -> int:
	...
	for alert in frappe.get_all(
		...
		rule = frappe.get_doc("K12 Alert Rule", alert.rule)
		actions = list(rule.actions_table)
		try:
			current = next(
...
def run_rules(rule: str = None, persona: str = None):
	...
	rules = (
		[frappe.get_doc("K12 Alert Rule", rule)]
		if rule
		else [
			frappe.get_doc("K12 Alert Rule", r.name)
			for r in frappe.get_all("K12 Alert Rule", filters={"enabled": 1}, fields=["name"])
		]
	)
	if not rules:
		return fail(
...
def run_rules_scheduled():
	...
	for r in frappe.get_all("K12 Alert Rule", filters={"enabled": 1}, fields=["name"]):
		try:
			evaluate_rule(frappe.get_doc("K12 Alert Rule", r.name))
		except Exception:
...
def get_rule(rule: str, persona: str = None):
	doc = frappe.get_doc("K12 Alert Rule", rule)
	return {
		"id": doc.name,
		"name": doc.rule_name,
		"trigger": doc.trigger,
...
def save_rule(payload: str | dict, persona: str = None):
	...
	doc = (
		frappe.get_doc("K12 Alert Rule", data["id"])
		if data.get("id")
		else frappe.new_doc("K12 Alert Rule")
	)
...
def delete_rule(rule: str, persona: str = None):
	...
	frappe.delete_doc("K12 Alert Rule", rule, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": rule},
...
def list_alerts(
	...
	total = frappe.db.count("K12 Student Alert", filters)
...
def _pages_of(alert: str) -> list[str]:
	...
	notes = frappe.db.get_value("K12 Student Alert", alert, "resolution_notes") or ""
	return [p.strip() for p in notes.split(",") if p.strip() in BLOCKABLE_PAGES]


@frappe.whitelist()
@k12_endpoint(ROLE_STUDENT, ROLE_PARENT, ROLE_ADMIN, ROLE_SECRETARY, ROLE_TEACHER)
