...
def get_plan(plan: str, persona: str = None):
	...
	doc = frappe.get_doc("K12 Timetable Plan", plan)
...
def save_plan(payload: str | dict, persona: str = None):
	...
	plan_id = data.get("id")
	doc = (
		frappe.get_doc("K12 Timetable Plan", plan_id)
		if plan_id
		else frappe.new_doc("K12 Timetable Plan")
	)
...
def delete_plan(plan: str, persona: str = None):
	frappe.delete_doc("K12 Timetable Plan", plan, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": plan},
