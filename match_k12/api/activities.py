...
def _eligible_activities(persona: str, scope: dict) -> list[str] | None:
	...
	names = set(frappe.get_all("K12 Activity", filters={"target_audience": "All"}, pluck="name"))
	if programs:
		names |= set(
			frappe.get_all(
				"K12 Activity",
...
def list_activities(
	...
	total = frappe.db.count("K12 Activity", filters)
...
def save_activity(payload: str | dict, persona: str = None):
	...
	doc = (
		frappe.get_doc("K12 Activity", activity_id)
		if activity_id
		else frappe.new_doc("K12 Activity")
	)
...
def delete_activity(activity: str, persona: str = None):
	...
	enrolled = frappe.db.count(
		"K12 Activity Enrolment", {"activity": activity, "status": ["!=", "Withdrawn"]}
	)
	if enrolled:
		return fail(
	...
	frappe.delete_doc("K12 Activity", activity, ignore_permissions=True)
	frappe.db.commit()
	return {
		"success": True,
		"data": {"id": activity},
...
def register(activity: str, student: str = None, persona: str = None):
	...
	existing = frappe.db.get_value(
		"K12 Activity Enrolment", {"activity": activity, "student": student}, "name"
	)
	if existing:
		row = frappe.get_doc("K12 Activity Enrolment", existing)
		...
		enrolment = frappe.new_doc("K12 Activity Enrolment")
...
def withdraw(enrolment: str, persona: str = None):
	...
	row = frappe.get_doc("K12 Activity Enrolment", enrolment)
...
def _promote_next(activity: str) -> str | None:
	...
	if not nxt:
		return None
	doc = frappe.get_doc("K12 Activity Enrolment", nxt[0].name)
...
@frappe.whitelist()
@k12_endpoint(ROLE_PARENT, ROLE_ADMIN, ROLE_SECRETARY)
